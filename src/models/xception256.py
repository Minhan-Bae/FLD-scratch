...
class EntryBlock(nn.Module):
    def __init__(self):
        ...
        self.conv5_direct = nn.Sequential(
...
class ExitBlock(nn.Module):
    def __init__(self):
        ...
        self.direct = nn.Sequential(
            nn.Conv2d(512, 1024, 1, stride = 2),
            nn.BatchNorm2d(1024)
        )

        self.conv = nn.Sequential(
            DepthewiseSeperableConv2d(1024, 1024, 3, padding = 1),
            nn.BatchNorm2d(1024),
            nn.LeakyReLU(0.2),
            DepthewiseSeperableConv2d(1024, 2048, 3, padding = 1),
            nn.BatchNorm2d(2048),
            nn.LeakyReLU(0.2)
        )
...
def xception_Net_54(pretrained=None, num_classes=27*2):
